...
export class FwProtheusModel {
	...
	public put() {
		...
		return this.fwModelService.put(body).pipe(tap(), catchError(this.handleError));
	}
	...
	public getModel(modelId: string): Model {
		...
		if (this.resources != undefined) {
			...
			if (this.models.find(model => model.id === modelId) == undefined) this.AddModel(modelId, 'FIELDS');
...
export class Model {
	...
	getModel(modelId: string): Model {
		...
		if (
			modelSup.modeltype == 'GRID' &&
			modelSup.items[this.linePos].models.find(model => model.id === modelId) == undefined
		)
			this.AddModel(modelId, 'GRID');
		...
		if (modelSup.modeltype == 'FIELDS') {
			model = this.models.find(model => model.id === modelId) || new Model();
		} else model = this.items[this.linePos].models.find(model => model.id === modelId) || new Model();
	...
	public setValue(fieldId: string, value: string): void {
		if (this.modeltype == 'FIELDS') {
			if (this.fields.find(field => field.id === fieldId) == undefined) this.addField(fieldId);
			...
			if (this.items[this.linePos].fields.find(field => field.id === fieldId) == undefined) this.addField(fieldId);
