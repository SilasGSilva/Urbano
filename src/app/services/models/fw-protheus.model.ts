...
export class FwProtheusModel {
  ...
  public getModel(modelId: string): Model {
    ...
    if (this.resources != undefined) {
      ...
      const model =
        this.models.find(model => model.id === modelId) || new Model();
...
export class Model {
  ...
  getModel(modelId: string): Model {
    ...
    if (
      modelSup.modeltype == 'FIELDS' &&
      this.models.find(model => model.id === modelId) == undefined
    )
      this.AddModel(modelId, 'GRID');

    if (
      modelSup.modeltype == 'GRID' &&
      modelSup.items[this.linePos].models.find(model => model.id === modelId) ==
        undefined
    )
      this.AddModel(modelId, 'GRID');
    ...
    if (modelSup.modeltype == 'FIELDS') {
      ...
      model =
        this.items[this.linePos].models.find(model => model.id === modelId) ||
        new Model();
  ...
  public setValue(fieldId: string, value: string): void {
    if (this.modeltype == 'FIELDS') {
      ...
      if (
        this.items[this.linePos].fields.find(field => field.id === fieldId) ==
        undefined
      )
        this.addField(fieldId);

      const field = this.items[this.linePos].fields.find(
