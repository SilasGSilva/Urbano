...
export class ViewMotoristaComponent {
    ...
    async getMotorista() {
        ...
        this.fwModel.get(params).subscribe({
            next: (data: any) => {
                this.titulo =
                    FindValueByName(data.models[0].fields, 'GYG_CODIGO') +
                    ' - ' +
                    FindValueByName(data.models[0].fields, 'GYG_NOME');
                ...
                this.viewMotorista = {
                    ...
                    codigo: FindValueByName(
                        data.models[0].fields,
                        'GYG_CODIGO'
                    ),
                    matricula: FindValueByName(
                        data.models[0].fields,
                        'GYG_FUNCIO'
                    ),
                    ...
                    dataVencimentoCNH: '',
                    tipoRecurso:
                        FindValueByName(data.models[0].fields, 'GYG_RECCOD') +
                        ' - ' +
                        FindValueByName(data.models[0].fields, 'GYG_DESREC'),
                    funcao:
                        FindValueByName(data.models[0].fields, 'GYG_FUNCOD') +
                        ' - ' +
                        FindValueByName(data.models[0].fields, 'GYG_FUNDES'),
                    turno: descTurno,
                    status,
                };
            },
    ...
    editar() {
        this.router.navigateByUrl(
            `motorista/detMotorista/editar/${this.filialMotorista}/${this.pkMotorista}`
        );
    }
