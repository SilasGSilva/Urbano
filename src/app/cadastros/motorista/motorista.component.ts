...
export class MotoristaComponent implements OnInit {
  ...
  public breadcrumb: PoBreadcrumb = {
    items: [
      ...
      { label: 'Motoristas/Colaboradores' },
    ],
  };
  ...
  setFilters() {
    ...
    if (this.cmbRecurso.selectedOption != undefined) {
      ...
      this.filters +=
        "GYG_RECCOD='" + this.cmbRecurso.selectedOption.value + "'";
    }
    if (this.cmbMotorista != undefined) {
      if (this.cmbMotorista.selectedOption != undefined) {
        ...
        this.filters +=
          "GYG_CODIGO ='" + this.cmbMotorista.selectedOption.value + "'";
        this.filters +=
          " AND GYG_FILIAL ='" +
          this.cmbMotorista['visibleOptions'][0].filial +
          "'";
      }
    }
    if (this.cmbTurno.selectedOption != undefined) {
      ...
      this.filters +=
        " GYG_TURNO LIKE '%" + this.cmbTurno.selectedOption.value + "%'";
    }
  ...
  getMotoristas() {
    ...
    this.fwModel.get(params).subscribe(() => {
      this.fwModel.resources.forEach((resource: Resource) => {
        ...
        motorista.filial = resource
          .getModel('GYGMASTER')
          .getValue('GYG_FILIAL');
        motorista.pk = resource.pk;
        motorista.matricula = resource
          .getModel('GYGMASTER')
          .getValue('GYG_FUNCIO');
        motorista.descMotorista = resource
          .getModel('GYGMASTER')
          .getValue('GYG_NOME');
        motorista.codRecurso = resource
          .getModel('GYGMASTER')
          .getValue('GYG_RECCOD');
        motorista.descRecurso =
          motorista.codRecurso +
          ' - ' +
          resource.getModel('GYGMASTER').getValue('GYG_DESREC');
  ...
  editar(item: any) {
    this.router.navigate(
      ['./detMotorista', 'editar', btoa(item.filial), item.pk],
  ...
  visualizar(item: any) {
    this.router.navigate(
      ['./viewMotorista', 'visualizar', btoa(item.filial), item.pk],
      { relativeTo: this.route }
    );
  }
  /**
   ...
   */
  incluir() {
    this.router.navigate(['./detMotorista', 'incluir'], {
      relativeTo: this.route,
    });
  }
}
