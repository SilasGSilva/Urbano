...
export class DetMotoristaComponent implements OnInit {
	...
	saveMotorista(isSaveNew: boolean = false) {
		...
		if (isSubmitable) {
			...
			this.fwModel
				.getModel('GYGMASTER')
				.setValue('GYG_FUNCIO', ChangeUndefinedToEmpty(this.motoristaForm.value.codMatricula));
			this.fwModel.getModel('GYGMASTER').setValue('GYG_NOME', ChangeUndefinedToEmpty(this.motoristaForm.value.nome));
			this.fwModel
				.getModel('GYGMASTER')
				.setValue(
					'GYG_DTNASC',
					dataNascimento != '' ? this.motoristaForm.value.dataNascimento.replace(/-/g, '') : ''
				);
			...
			if (tipoDocumento !== null) {
				if (tipoDocumento == '1') {
					...
					this.fwModel
						.getModel('GYGMASTER')
						.setValue('GYG_CPF', this.motoristaForm.value.numeroDocumento.replace(/[.-]/g, ''));
				}
			}
			this.fwModel
				.getModel('GYGMASTER')
				.setValue('GYG_RECCOD', ChangeUndefinedToEmpty(this.motoristaForm.value.codTipoRecurso));
			this.fwModel.getModel('GYGMASTER').setValue('GYG_FUNCOD', this.motoristaForm.value.codFuncao);
			this.fwModel
				.getModel('GYGMASTER')
				.setValue('GYG_TURNO', ChangeUndefinedToEmpty(this.motoristaForm.value.turno.join('')));
			this.fwModel
				.getModel('GYGMASTER')
				.setValue('GYG_STATUS', ChangeUndefinedToEmpty(this.motoristaForm.value.status));
