...
export class DetMotoristaComponent implements OnInit {
    ...
    createForm(): any {
        ...
        this.motoristaForm = this.formBuilder.group({
            ...
            codMatricula: [
                motorista.codMatricula,
                Validators.compose([Validators.required]),
            ],
            descMatricula: [motorista.descMatricula],
            codTipoRecurso: [
                motorista.codTipoRecurso,
                Validators.compose([Validators.required]),
            ],
        ...
        this.fwModel.get(params).subscribe({
            next: (data: any) => {
                ...
                this.motoristaForm.patchValue({
                    ...
                    codFuncao: FindValueByName(
                        data.models[0].fields,
                        'GYG_FUNCOD'
                    ),
                    descFuncao: FindValueByName(
                        data.models[0].fields,
                        'GYG_FUNDES'
                    ),
                    status: status,
                    codMatricula: FindValueByName(
                        data.models[0].fields,
                        'GYG_FUNCIO'
                    ),
                    descMatricula: FindValueByName(
                        data.models[0].fields,
                        'GYG_NOME'
                    ),
                    codTipoRecurso: FindValueByName(
                        data.models[0].fields,
                        'GYG_RECCOD'
                    ),
                    descTipoRecurso: FindValueByName(
                        data.models[0].fields,
                        'GYG_DESREC'
                    ),
                    turno: turno.split(''),
                    ...
                    codigoMotorista: FindValueByName(
                        data.models[0].fields,
                        'GYG_CODIGO'
                    ),
                    dataNascimento: MakeDate(
                        FindValueByName(data.models[0].fields, 'GYG_DTNASC'),
                        'yyyy-mm-dd'
                    ),
                });
            },
        ...
        if (this.comboMatricula != undefined && newValue != undefined) {
            this.comboMatricula['visibleOptions'].forEach((item: any) => {
                if (item.selected) {
                    ...
                    if (
                        isNullOrUndefined(this.motoristaForm.value.nome) &&
                        nome !== ''
                    ) {
                        this.motoristaForm.patchValue({
                            nome: nome,
                    ...
                    if (
                        isNullOrUndefined(
                            this.motoristaForm.value.numeroDocumento
                        ) &&
                        cpf != ''
                    ) {
                        this.motoristaForm.patchValue({
    ...
    saveMotorista(isSaveNew: boolean = false) {
        ...
        let dataNascimento = ChangeUndefinedToEmpty(
            this.motoristaForm.value.dataNascimento
        );
        ...
        if (isSubmitable) {
            ...
            this.fwModel
                ...
                .setValue(
                    'GYG_FUNCIO',
                    ChangeUndefinedToEmpty(
                        this.motoristaForm.value.codMatricula
                    )
                );
            this.fwModel
                .getModel('GYGMASTER')
                .setValue(
                    'GYG_NOME',
                    ChangeUndefinedToEmpty(this.motoristaForm.value.nome)
                );
            this.fwModel
                .getModel('GYGMASTER')
                .setValue(
                    'GYG_DTNASC',
                    dataNascimento != ''
                        ? this.motoristaForm.value.dataNascimento.replace(
                              /-/g,
                              ''
                          )
                        : ''
            ...
            this.fwModel
                .getModel('GYGMASTER')
                .setValue('GYG_TPDOC', ChangeUndefinedToEmpty(tipoDocumento));
            if (tipoDocumento !== null) {
                if (tipoDocumento == '1') {
                    ...
                    this.fwModel
                        ...
                        .setValue(
                            'GYG_CPF',
                            this.motoristaForm.value.numeroDocumento.replace(
                                /[.-]/g,
                                ''
                            )
                        );
            ...
            this.fwModel
                ...
                .setValue(
                    'GYG_RECCOD',
                    ChangeUndefinedToEmpty(
                        this.motoristaForm.value.codTipoRecurso
                    )
                );
            this.fwModel
                .getModel('GYGMASTER')
                .setValue('GYG_FUNCOD', this.motoristaForm.value.codFuncao);
            this.fwModel
                .getModel('GYGMASTER')
                .setValue(
                    'GYG_TURNO',
                    ChangeUndefinedToEmpty(
                        this.motoristaForm.value.turno.join('')
                    )
                );
            this.fwModel
                .getModel('GYGMASTER')
                .setValue(
                    'GYG_STATUS',
                    ChangeUndefinedToEmpty(this.motoristaForm.value.status)
                );
