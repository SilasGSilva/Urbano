			...
			this.fwModel.get().subscribe({
				next: () => {
					...
					this.comboFormService.setFilterUf(
						"AND UPPER(CC2_EST) LIKE UPPER('%" + this.fwModel.getModel('GI1MASTER').getValue('GI1_UF') + "%'))"
					);
	...
	saveLocalidade(isSaveNew: boolean = false): void {
		...
		if (isSubmitable) {
			...
			if (this.localidadesForm.value.tipoLocalidade) {
				this.fwModel
					.getModel('GI1MASTER')
					.setValue('GI1_TPLOC', ChangeUndefinedToEmpty(this.localidadesForm.value.tipoLocalidade.join('')));
			} else {
			...
			this.fwModel
				.getModel('GI1MASTER')
				.setValue('GI1_STATUS', ChangeUndefinedToEmpty(this.localidadesForm.value.status));
