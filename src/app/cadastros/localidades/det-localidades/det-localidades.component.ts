...
import { ChangeUndefinedToEmpty, FindValueByName, isNullOrUndefined } from 'src/app/services/functions/util.function';
...
export class DetLocalidadesComponent implements OnInit {
    ...
    initializeForm(): void {
        ...
        this.localidadesForm = this.formBuilder.group({
            descricao: [localidade.descricao, Validators.compose([Validators.required])],
            cep: [localidade.cep],
    ...
    vldDetNotify(): void {
        const listNotification: VldFormStruct[] = this.apiService.validateForm(this.localidadesForm);
