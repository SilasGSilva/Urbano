...
export class ViewLocalidadesComponent {
    ...
    async getLocalidade() {
        ...
        this.fwModel.get().subscribe({
            next: () => {
                let tpLoc = this.fwModel
                    .getModel('GI1MASTER')
                    .getValue('GI1_TPLOC');
                let codLoc = this.fwModel
                    .getModel('GI1MASTER')
                    .getValue('GI1_COD');
                let descLoc = this.fwModel
                    .getModel('GI1MASTER')
                    .getValue('GI1_DESCRI');
                let status = this.fwModel
                    .getModel('GI1MASTER')
                    .getValue('GI1_STATUS');
                let descTipo = '';
    ...
    editar() {
        this.router.navigateByUrl(
            `localidades/detLocalidades/editar/${this.pkLocalidade}`
        );
    }
