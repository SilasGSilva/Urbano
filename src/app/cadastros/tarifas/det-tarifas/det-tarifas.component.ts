...
import { poLookUpFormasDePagamento, OrgaoConcessorComboService } from 'src/app/services/adaptors/wsurbano-adapter.service';
...
export class DetTarifasComponent {
	...
	createForm(): any {
		...
		this.tarifaForm = this._formBuilder.group({
			...
			formasDePagamento: [tarifa.formasDePagamento, Validators.compose([Validators.required])],
		});
	}
		...
		if (!this.editView) {
			...
			if (generateHistory) {
				...
				setTimeout(() => {
					...
					this.changeLoading();
					this._router.navigate(['tarifas']);
					this._poNotification.success('Tarifa alterada e histórico gerado com sucesso!');
				}, 1000);
			} else {
				this.changeLoading();
				setTimeout(() => {
