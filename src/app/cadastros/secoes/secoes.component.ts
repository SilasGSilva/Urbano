...
export class SecoesComponent {
	...
	setFilters() {
		...
		//filtros
		if (this.secaoFilterCombo !== undefined && this.secaoFilterCombo.selectedOption !== undefined) {
			if (this.filters != '') {
				this.filters += ' AND ';
			}
			this.filters += " GI1_COD = '" + this.secaoFilterCombo.selectedOption.value + "' ";
		}
		// if (
	 ...
	 *******************************************************************************/
	editSecao(event: any) {
		this._router.navigate(['secoes/detSecoes', 'editar', btoa(event.pk), event.pk]);
	}
