...
import { PaymentMethodColumns, PaymentMethodModel } from './formas-de-pagamento-struct';
...
export class FormasDePagamentoComponent {
	...
	public breadcrumb: PoBreadcrumb = {
		items: [{ label: 'Fretamento Urbano', link: '/' }, { label: 'Formas de pagamento' }],
	};
	constructor(
	...
	getFormaPagamento() {
		...
		this.fwModel.get(params).subscribe(() => {
			this.fwModel.resources.forEach((resource: Resource) => {
				...
				paymentMethod.codPayment = resource.getModel('H6RMASTER').getValue('H6R_CODIGO');
				...
				paymentMethod.descPayment = resource.getModel('H6RMASTER').getValue('H6R_DESCRI');
