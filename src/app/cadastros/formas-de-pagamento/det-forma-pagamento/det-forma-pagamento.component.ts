...
export class DetFormaPagamentoComponent implements OnInit {
    ...
    saveAction(isSaveNew: boolean = false) {
        ...
        if (isSubmitable) {
            ...
            this.fwModel
                ...
                .setValue(
                    'H6R_CODIGO',
                    ChangeUndefinedToEmpty(this.formaPagForm.value.codigo)
                );
            this.fwModel
                .getModel('H6RMASTER')
                .setValue(
                    'H6R_DESCRI',
                    ChangeUndefinedToEmpty(this.formaPagForm.value.descricao)
                );
    ...
    getFormaPagamento() {
        ...
        this.fwModel.get(params).subscribe({
            next: (data: any) => {
                let codigo: string = FindValueByName(
                    data.models[0].fields,
                    'H6R_CODIGO'
                );
                let descricao: string = FindValueByName(
                    data.models[0].fields,
                    'H6R_DESCRI'
                );
