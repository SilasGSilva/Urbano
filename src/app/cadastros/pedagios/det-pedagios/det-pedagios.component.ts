...
import { PedagioForm, PedagioStruct, verifyDatePickerRange } from './det-pedagios.struct';
...
export class DetPedagiosComponent {
	...
	createForm(): any {
		...
		this.pedagioForm = this._formBuilder.group({
			...
			vigencia: [pedagio.vigencia, Validators.compose([Validators.required]), verifyDatePickerRange],
		});
	}
		...
		if (!this.editView) {
			...
			if (generateHistory) {
				...
				setTimeout(() => {
					...
					this.changeLoading();
					this._router.navigate(['pedagios']);
					this._poNotification.success('Pedágio alterado e histórico gerado com sucesso!');
				}, 1000);
			} else {
				this.changeLoading();
				setTimeout(() => {
